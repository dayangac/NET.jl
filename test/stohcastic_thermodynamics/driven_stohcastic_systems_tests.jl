...
@testset "DrivenStohcasticSystems" begin
    ...
    # Testing the same functionality using StochasticOnsagerRelations module
    @testset "StochasticOnsagerMatrix construction" begin
        L = zeros(2, 2, 3)
        L[:, :, 1] = [1.0 0.5; 0.5 2.0]
        matrix = NET.StochasticOnsagerRelations.StochasticOnsagerMatrix(L)
        @test matrix.L == L
    end
    ...
    @testset "compute_stochastic_fluxes" begin
        ...
        J = NET.StochasticOnsagerRelations.compute_stochastic_fluxes(matrix, F, noise)
        @test size(J) == size(F)
    end
end
